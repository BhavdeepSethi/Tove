		...
		<script type="text/javascript">
				$(document).ready(function(){
						$(".basicRating").jRating({
      					...
      					/*
      					$('.hasTooltip').qtip({ // Grab some elements to apply the tooltip to
  							  content: {
        					   text: $(this).next('div')
    						 },
    						  position: {
		...
		foreach ($files as $entry) {
			...
			<div class="one hasTooltip">
				...
				<div class="two hidden">
						<div style="padding:10px; background: #3A79B8; color:white;">
							<div style="font-size:130%; margin-top:3px; ">
								<?= $entry["title"]; ?>
							</div>
						...
						<!--<div class="progress" style="width: <?= $entry['rating'] ?>%;"></div>-->
							...
							<div class="fp_yearRateTime"><?= $entry["year"]; ?> 
								<span class="fp_rated"><?= formatRating($entry["contentRating"], "white"); ?></span>
								<?= $entry["runtime"]; ?>mins 
							</div>
						</div>
						<div class="ratingBox">
